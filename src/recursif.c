...
/*   Updated: 2017/06/29 16:01:42 by ibtraore         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv1.h"
#define ADDCOL(x, a, b, c, d) add_col(t_color a1, t_color a2, t_color a3);
...
t_color		add_col(t_color amb, t_color a1, t_color a2, t_color a3)
{
	t_color	color;
	int		tmp[3];
...
t_color		recursif_trajet(t_ray ray, int depth, t_env *env)
{
	...
	else if ((t = find_closest_t(env->obj, depth, env))  && hit_obj)
	{
		hit.t = t;
